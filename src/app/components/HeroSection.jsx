...
const HeroSection = () => {
  ...
  return (
    <>
        ...
        <div className="xl:w-[40%] flex xl:text-left text-center xl:items-start flex-col items-center">
          ...
          <div className="relative xl:mx-0 mx-auto w-full h-fit mt-12">
            ...
            <button 
              ...
              className={`bg-[#6965FD] cursor-pointer px-4 sm:px-6 py-2.5 absolute right-1.5 top-1/2 -translate-y-1/2 rounded-full text-[#FEFEFF] sm:text-[16px] text-sm sm:font-semibold ${!address ? 'opacity-50 cursor-not-allowed' : ''}`}
              disabled={!address} // Disables the button when address is empty
            >
              Get Valuation
            </button>
          </div>
      ...
      {steps && <EveluationSteps address={address} />} {/* Pass the address as a prop */}
    </>
  )
}
